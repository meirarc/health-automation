...
def search_pubmed(query: str, max_results: int = 3) -> str:
    ...
    try:
        ...
        if not article_ids:
            return json.dumps({"error": f"Nenhum estudo encontrado para '{query}'."}, ensure_ascii=False)
        ...
        for article_id in article_ids:
            ...
            abstract_text = abstract_response.text.strip()[:500] if abstract_response.status_code == 200 else "Resumo não disponível"
        ...
        return json.dumps({"error": f"Erro ao acessar a API do PubMed: {str(e)}"}, ensure_ascii=False)
...
def summarize_pubmed_results(response: str) -> str:
    ...
    try:
        ...
        for article in articles[:3]:  # 🔹 Garante que usamos no máximo 3 artigos
            title = article.get("title", "Título não disponível")
            abstract = article.get("abstract", "Resumo não disponível")[:300]  # 🔹 Corta para 300 caracteres
            link = article.get("link", "")

            summarized_articles.append(f"📌 **{title}**\nResumo: {abstract}...\n🔗 [Leia mais]({link})\n")
