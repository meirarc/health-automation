...
def search_supplements_in_spain(query: str, max_results: int = 5) -> str:
    ...
    try:
        search_query = (
            f"{query} suplemento OR vitaminas site:amazon.es OR site:iherb.com"
        )

        # 🔹 Executa a busca
        raw_results = search_best_supplements.run(
            query=search_query, gl="es", num=max_results
        )

        # 🔹 Debug: Mostrar a resposta completa do Serper
        print(
            f"🔍 Resposta do Serper para '{query}': {json.dumps(raw_results, indent=2, ensure_ascii=False)}"
        )
        ...
        try:
            ...
            return json.dumps(
                {"error": "Erro ao decodificar resposta da API Serper."},
                ensure_ascii=False,
            )
        ...
        if (
            ...
            or "organic" not in results
            or not isinstance(results["organic"], list)
            or not results["organic"]
        ):
            return json.dumps(
                {
                    "error": f"Nenhum produto encontrado para '{query}'. Tente buscar por outro nome ou verificar disponibilidade."
                },
                ensure_ascii=False,
            )
        ...
        return (
            json.dumps(supplements[:max_results], indent=2, ensure_ascii=False)
            if supplements
            else json.dumps(
                {"error": f"Nenhum produto encontrado para '{query}'."},
                ensure_ascii=False,
            )
        )
        ...
        return json.dumps(
            {"error": f"Erro ao buscar suplementos: {str(e)}"}, ensure_ascii=False
        )
...
def fetch_recommended_dosages(supplement_name: str, health_condition: str) -> str:
    ...
    try:
        ...
        if "organic" not in results or not results["organic"]:
            return json.dumps(
                {
                    "error": f"Nenhuma informação de dosagem encontrada para '{supplement_name}' com {health_condition}."
                },
                ensure_ascii=False,
            )
        ...
        return (
            json.dumps(dosage_info[:3], indent=2, ensure_ascii=False)
            if dosage_info
            else json.dumps({"error": "Nenhuma informação encontrada."})
        )

    except Exception as e:
        return json.dumps(
            {"error": f"Erro ao buscar dosagem: {str(e)}"}, ensure_ascii=False
        )
