...
def search_supplement_products(supplement_name: str, max_results: int = 5) -> str:
    ...
    try:
        ...
        amazon_response = requests.get(
            AMAZON_API_URL, params={"query": supplement_name, "limit": max_results}
        )
        amazon_response.raise_for_status()
        amazon_results = amazon_response.json().get("products", [])

        # 🔹 Busca no iHerb
        iherb_response = requests.get(
            IHERB_API_URL, params={"query": supplement_name, "limit": max_results}
        )
        ...
        return (
            json.dumps(results[:max_results], indent=2, ensure_ascii=False)
            if results
            else json.dumps({"error": "Nenhum produto encontrado."})
        )
